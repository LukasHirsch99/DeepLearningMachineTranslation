...
if __name__ == "__main__":
    ...
    print("\n=== Key Properties ===")
    print(f"1. Same positional encoding for all batches: {torch.allclose(output[0], output[1])}... No! Because embeddings differ")
    ...
    print(f"   Position 0 != Position 1: {not torch.allclose(pe_values_pos0, pe_values_pos1)}")
    ...
    learned_pe = LearnedPositionalEncoding(d_model=d_model, max_len=max_len, dropout=0.1)
    ...
    print(f"Number of learnable parameters: {sum(p.numel() for p in learned_pe.parameters())}")
